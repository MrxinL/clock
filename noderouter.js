...
Router.get('/user/images',(req,res)=>{
  ...
  console.log(typeof query.collect.toLowerCase(),query.collect.toLowerCase(),'-----',collect)
  ...
  if(per_page === '4'){
       ...
       if(collect){
         ...
         sql=`select count(*) from material where is_collected=${collect};
         select *from material where is_collected=${collect} order by id desc limit ${start},${per_page}`
       }else{
